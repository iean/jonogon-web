...
import { InputOtpChangeEvent } from './input-otp.interface';
      ...
      <ng-container *ngIf="inputTemplate">
        <ng-container
          *ngTemplateOutlet="
            inputTemplate;
            context: {
              $implicit: getToken(i - 1),
              events: getTemplateEvents(i - 1),
              index: i
            }
          "></ng-container>
      </ng-container>
...
export class InputOtp implements AfterContentInit {
   ...
   * @group Emits
   */
  @Output() onChange: EventEmitter<InputOtpChangeEvent> =
    new EventEmitter<InputOtpChangeEvent>();
  /**
  ...
  getTemplateEvents(index: number) {
    return {
      ...
      keydown: (event: Event) => this.onKeyDown(event),
      focus: (event: Event) => this.onFocus.emit(event),
      blur: (event: Event) => this.onBlur.emit(event),
      paste: (event: Event) => this.onPaste(event),
    };
  }
    ...
    switch (event.key) {
      ...
      default:
        if (
          ...
          (this.tokens.join('').length >= this.length &&
            event.code !== 'Delete')
        ) {
          event.preventDefault();
        }
